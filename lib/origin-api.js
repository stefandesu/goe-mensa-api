...
const loadDishes = (db) => {
  ...
  Promise.resolve().then(() => {
    ...
    for (let language of ["de", "en"]) {
      for (let mensa of mensen) {
        for (let priceType of priceTypes) {
          let uri = baseUri.replace("{{language}}", language).replace("{{date}}", date()).replace("{{mensa}}", mensa._id.toLowerCase().split(" ").join("_"))
          promises.push(rp({
            uri,
            transform: body => cheerio.load(body)
          }).then($ => {
            return {
              $, mensa, priceType, language
            }
          }))
        }
      }
    ...
    for (let { $, mensa, priceType, language } of results) {
      $("tr[class=odd],tr[class=even]").each((index, element) => {
        ...
        if (category) {
          ...
          if (dish) {
            ...
            // And title/description for the language
            dish.title[language] = dishTitle
            dish.description[language] = dishDescription
          } else {
